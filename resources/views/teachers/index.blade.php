        ...
        <tbody>
            ...
            <tr >
             ...
             <td>
                ...
                <input style="width: 200px;" type="text" class="form-control" name="Accuracy" id="Accuracy-{{ $item->id }}" value="{{ $item->Accuracy }}" disabled>
                </td>
                <td>
                <input style="width: 200px;" type="text" class="form-control" name="birthdate" id="birthdate-{{ $item->id }}" value="{{ $item->birthdate }}" disabled>
                </td>
                <td>
                <input style="width: 200px;" type="text" class="form-control" name="adress" id="adress-{{ $item->id }}" value="{{ $item->adress }}" disabled>
                </td>
                <td>
                <input style="width: 200px;" type="text" class="form-control" name="parent_phone1" id="parent_phone1-{{ $item->id }}" value="{{ $item->parent_phone1 }}" disabled>
                </td>
                <td>
                <input style="width: 200px;" type="text" class="form-control" name="parent_phone2" id="parent_phone2-{{ $item->id }}" value="{{ $item->parent_phone2 }}" disabled>
                </td>
                <td>                
                ...
                <td>
        <form action="{{ route('teacher.destroy',$item->id ) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من رغبتك في حذف هذا المنتج؟')">
    @csrf
    @method('DELETE')
...
function enableEdit(rowId) {
    ...
    var descInput = document.getElementById('Accuracy-' + rowId);
    var d2escInput = document.getElementById('birthdate-' + rowId);
    var d2es2cInput = document.getElementById('adress-' + rowId);
    var d3escInput = document.getElementById('parent_phone1-' + rowId);
    var d4escInput = document.getElementById('parent_phone2-' + rowId);
